...
function cleanData(data) {
  ...
  return {
    ...
    // Fix column name mapping - handle potential whitespace issues
    ...
    material: (data.Material || data['Material '] || '').trim(),
    site: (data.Site || data['Site '] || data['Site Name '] || data['Site Name'] || '').trim(),
    vehicleId: (data['Vehicle ID'] || data['Vehicle ID '] || '').trim(),
    vehicles: (data.Vehicles || data['Vehicles '] || '').trim(),
    uom: (data.UOM || data['UOM '] || '').trim(),
  };
}
...
async function finalImport() {
  try {
    console.log('🚀 Starting Final CSV Data Import...');
    ...
    console.log('Raw data keys:', Object.keys(purchaseData[0]));
    ...
    console.log('Raw data keys:', Object.keys(expenseData[0]));
