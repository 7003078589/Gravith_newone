...
import MaterialSubCategoryForm from './forms/MaterialSubCategoryForm';
...
export function MastersPage() {
  ...
  const [subCategoryData, setSubCategoryData] =
    useState<MaterialSubCategoryItem[]>(mockSubCategories);
  const [subCategorySearchQuery, setSubCategorySearchQuery] = useState<string>('');
  const [subCategoryFilter, setSubCategoryFilter] = useState<string>('all');
  const [subCategoryStatusFilter, setSubCategoryStatusFilter] = useState<string>('all');
  const [isSubCategoryDialogOpen, setIsSubCategoryDialogOpen] = useState(false);
  const [editingSubCategory, setEditingSubCategory] = useState<MaterialSubCategoryItem | null>(
    null,
  );
  ...
  const subCategoryTableState = useTableState({
    initialSortField: 'code',
    initialSortDirection: 'asc',
    initialItemsPerPage: 10,
  });
  ...
  const handleSubCategorySubmit = (
    formData: Omit<MaterialSubCategoryItem, 'id' | 'createdAt' | 'updatedAt'>,
  ) => {
    if (editingSubCategory) {
      const updatedSubCategory: MaterialSubCategoryItem = {
        ...editingSubCategory,
        ...formData,
        updatedAt: new Date().toISOString(),
      };
      setSubCategoryData((prev) =>
        prev.map((subCat) => (subCat.id === editingSubCategory.id ? updatedSubCategory : subCat)),
      );
    } else {
      const newSubCategory: MaterialSubCategoryItem = {
        ...formData,
        id: (subCategoryData.length + 1).toString(),
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
  ...
  const handleEditSubCategory = (subCategory: MaterialSubCategoryItem) => {
    ...
    setSubCategoryData((prev) =>
      prev.map((subCat) =>
        subCat.id === subCategoryId
          ? { ...subCat, isActive: !subCat.isActive, updatedAt: new Date().toISOString() }
          : subCat,
      ),
    );
  };
  ...
  const filteredSubCategories = subCategoryData
    .filter((subCategory) => {
      const matchesSearch =
        ...
        subCategory.code.toLowerCase().includes(subCategorySearchQuery.toLowerCase()) ||
      ...
      const matchesStatus =
        subCategoryStatusFilter === 'all' ||
        (subCategoryStatusFilter === 'active' && subCategory.isActive) ||
        (subCategoryStatusFilter === 'inactive' && !subCategory.isActive);
      return matchesSearch && matchesCategory && matchesStatus;
    })
    .sort((a, b) => {
      const aValue = a[subCategoryTableState.sortField as keyof MaterialSubCategoryItem];
      const bValue = b[subCategoryTableState.sortField as keyof MaterialSubCategoryItem];
      if (typeof aValue === 'string' && typeof bValue === 'string') {
        return subCategoryTableState.sortDirection === 'asc'
          ? aValue.localeCompare(bValue)
          : bValue.localeCompare(aValue);
      }
      return 0;
  ...
  const paginatedSubCategories = filteredSubCategories.slice(
    (subCategoryTableState.currentPage - 1) * subCategoryTableState.itemsPerPage,
    subCategoryTableState.currentPage * subCategoryTableState.itemsPerPage,
  );
  ...
  const subCategoryTabContent = (
    <div className="space-y-6">
              ...
              <CardContent className="p-4 md:p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-2xl font-bold text-primary">{subCategoryData.length}</p>
                  </div>
                  <div className="h-12 w-12 bg-primary/20 rounded-lg flex items-center justify-center">
              ...
              <CardContent className="p-4 md:p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-2xl font-bold text-green-600">
                      {subCategoryData.filter((sc) => sc.isActive).length}
                    </p>
                  </div>
                  <div className="h-12 w-12 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center">
                    <CheckCircle2 className="h-6 w-6 text-green-600" />
      ...
      {/* Sub-Category Search and Filters */}
      <Card className="w-full overflow-hidden">
        <CardContent className="p-4 md:p-6">
          <div className="flex flex-col gap-4 w-full">
            <div className="flex flex-col lg:flex-row lg:items-center gap-4 w-full">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-1 min-w-0 w-full">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  ...
                  <Select
                    ...
                    onValueChange={setSubCategoryStatusFilter}
                  >
                    <SelectTrigger className="w-full sm:w-[150px]">
                      <SelectValue />
                    </SelectTrigger>
              ...
              <Dialog open={isSubCategoryDialogOpen} onOpenChange={setIsSubCategoryDialogOpen}>
                ...
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      {editingSubCategory
                        ? 'Update the material sub-category details below'
                        : 'Create a new material sub-category'}
                    </DialogDescription>
                  </DialogHeader>
                  <MaterialSubCategoryForm
                    onSubmit={handleSubCategorySubmit}
                    onCancel={() => {
                      setEditingSubCategory(null);
                      setIsSubCategoryDialogOpen(false);
                    }}
                    defaultValues={editingSubCategory || undefined}
      ...
      <Card className="w-full overflow-hidden">
        <CardContent className="p-0">
              ...
              <TableBody>
                {paginatedSubCategories.map((subCategory) => (
                  <TableRow key={subCategory.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline">{getCategoryNameById(subCategory.categoryId)}</Badge>
                    </TableCell>
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>{subCategory.isActive ? 'Deactivate' : 'Activate'}</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
          ...
          <div className="flex items-center justify-between p-4 border-t">
            <p className="text-sm text-muted-foreground">
              Showing{' '}
              {(subCategoryTableState.currentPage - 1) * subCategoryTableState.itemsPerPage + 1} to{' '}
              {Math.min(
                subCategoryTableState.currentPage * subCategoryTableState.itemsPerPage,
                filteredSubCategories.length,
              )}{' '}
              of {filteredSubCategories.length} sub-categories
            </p>
            <div className="flex gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() =>
                  subCategoryTableState.setCurrentPage(subCategoryTableState.currentPage - 1)
                }
                disabled={subCategoryTableState.currentPage === 1}
              >
                Previous
              </Button>
              <Button
                ...
                disabled={
                  subCategoryTableState.currentPage >=
                  Math.ceil(filteredSubCategories.length / subCategoryTableState.itemsPerPage)
                }
              >
                Next
              </Button>
