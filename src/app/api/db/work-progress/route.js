...
export async function GET(request) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, data: workProgressData, count: workProgressData.length, source: 'mock' }),
      { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } },
    );
  } catch (error) {
    return new Response(
      JSON.stringify({ success: false, error: 'Failed to load mock work progress', details: String(error?.message || error) }),
